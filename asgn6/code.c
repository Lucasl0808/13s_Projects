...
bool code_set_bit(Code *c, uint32_t i) {
    ...
    tempc
        = tempc
          | and; //tempc = c->bits[i] where location of i is zero makes sure there is a 1 in correct position

    c->bits[tempi]
        = c->bits[tempi] | tempc; //make a copy of c->bits[i] using | to add one to desired bit
    return true;
}
...
bool code_clr_bit(Code *c, uint32_t i) {
    ...
    tempc
        = 255
          - and; //tempc = 8 bits of 1's where desired position is set to 0 to ensure that when using & operator the desired bit is set to 0

    c->bits[tempi]
        = c->bits[tempi] & tempc; //make a copy of c->bits[i] using & to zero out desired bit
    return true;
}
...
bool code_get_bit(Code *c, uint32_t i) {
    ...
    tempc
        = c->bits[tempi]
          >> tempr; //tempc shifts right by the position of the desired number. Puts desired bit at the 0th index
