...
int main(int argc, char **argv){
	...
	while((opt = (getopt(argc, argv, "hi:o:v"))) != -1){
		...
		case 'h':
			...
			fprintf(stderr, "-o outfile     specify output file to write decompressed file from huffman decoding to\n");
			fprintf(stderr, "-v     prints file compression statistics to stderr. Prints compressed file size, decompressed file size, and space saving\n");
			return 1;
		case 'i':
	...
	read_bytes(infile, tree_dump, h.tree_size);	//read tree_size bytes given by header into tree_dump array
	
	Node *root = rebuild_tree(h.tree_size, tree_dump);	//root of the tree is the root of the rebuilt tree using tree_dump
	...
	if(verbose == true){
		...
		int saving = 100 * (1 -(infile_stat.st_size / outfile_stat.st_size));
